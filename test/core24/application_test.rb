...
class ApplicationTest < ActionDispatch::IntegrationTest
  ...
  def setup
    @fixture = reload_fixture 'QA-CORE-1-UA-012'
    # @fixtures = 
  end
  ...
  test 'a core test' do
    new_app = reload_fixture 'QA-CORE-1-UA-012'
    puts new_app[:response] 
    puts new_app[:response]["Determination Date"]
    new_app[:response]["Applicants"].each do |app|
       assert_equal app["Medicaid Eligible"], "N"
       puts app["Medicaid Household"]["MAGI as Percentage of FPL"]
       assert_equal app["Medicaid Household"]["MAGI as Percentage of FPL"], "84".to_i
    end
  end
...
# just for a single -- no need to run for all possible, and it slows 
  ...
  test 'A core Application' do
     new_app = Application.new('kajdsfkljsadf','application/json')
     refute_nil new_app.error
  end
